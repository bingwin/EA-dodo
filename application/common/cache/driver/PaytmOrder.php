...
use app\common\model\paytm\PaytmOrder as PaytmOrderModel;


class PaytmOrder extends Cache
{
    private $key = 'table:paytm:order:';
    private $set = 'table:paytm:order:set';
    private $orderSyncTime = 'order_sync_time';

    /** 获取属性信息
     * @param accountId 账号id
     * @param string $order_number paytm订单id
     * @param array $data
     * @return array|mixed
     */
...
//            $this->redis->hset($key, $order_number, json_encode($data));
    ...
    {
        $key = 'hash:PaytmOrderUpdateTime';
        $list = PaytmOrderModel::field('id,order_number,last_update_time')->where(['last_update_time' => ['gt', time() - 3*24*3600]])->select();
    ...
    {
        $this->redis->hSet('hash:PaytmOrderRefundLogs', $key, json_encode($data));
    }
    ...
    {
        if ($this->redis->hExists('hash:PaytmOrderRefundLogs', $key)) {
            return true;
        }
        return false;
    }
    ...
    {
        $this->redis->zAdd('hash:PaytmOrderLogs', $key, json_encode($data));
    }
    ...
    {
        $result = [];
        if($this->persistRedis->exists('hash:PaytmOrderLogs')) {
            $result = $this->persistRedis->zRange('hash:PaytmOrderLogs', $start, $end);
        }
        return $result;
    }

    /**
     * 拿取paytm账号最后下载订单更新的时间
    ...
    {
        $this->redis->zAdd('hash:PaytmOrderToLocalLogs', $key, json_encode($data));
    }
    ...
    {
        $result = [];
        if($this->persistRedis->exists('hash:PaytmOrderToLocalLogs')) {
            $result = $this->persistRedis->zRange('hash:PaytmOrderToLocalLogs', $start, $end);
        }
        return $result;
    }
    ...
    {
        $this->redis->zAdd('hash:PaytmSynchronousLogs', $key, json_encode($data));
    }
    ...
    {
        $result = [];
        if($this->persistRedis->exists('hash:PaytmSynchronousLogs')) {
            $result = $this->persistRedis->zRange('hash:PaytmSynchronousLogs', $start, $end);
        }
        return $result;
    }
