...
class maytmAbroadOrder extends ErpModel
{
    use ModelFilter;
    ...
    {
        $masterTable = "maytm_abroad_order";
        $detailTable = "paytm_order_detail";
        $partitionCache = Cache::store('Partition');
        if (!empty($order)) {
            $detailModel = new maytmAbroadOrderDetail();
            ...
            if (isset($order['order']['order_id'])) { // 启动事务
                ...
                try {
                    $itemIds = [];
                    if (!$order['order']['id']) {
                        $id = Db::name('maytm_abroad_order')->insert($order['order'], false, true);
    ...
    //     $masterTable = "maytm_abroad_order";
    //     //TODO:
    //     $detailTable = "maytm_abroad_order_detail";
    //     $partitionCache = Cache::store('Partition');
    //     if (!empty($order)) {
    //         //TODO:
    //         $detailModel = new maytmAbroadOrderDetail();
    //         // if(isset($order['order']['create_time'])) {
    //         //     if (!$partitionCache->getPartition('MaytmOrder', $order['order']['create_time'])) {
    //         //         $partitionCache->setPartition('MaytmOrder', $order['order']['create_time']);
    //         //     }
    //         // }
    ...
    //                 ];
    //                  //TODO:
    //                 Cache::store('MaytmOrder')->orderUpdateTime($order['order']['account_id'], $order['order']['order_id'], $info);
    //             } catch (\Exception $e) {
    //                 // 回滚事务
    //                 Db::rollback();
    //                 Cache::handler()->hSet('hash:PaytmOrderFailure', $order['order']['order_id'], $e->getMessage());
...
//        $this->update(['latest_ship_time' => $payment_time + $upload_time], ['id' => $orderitem['id']]);
    ...
    {
        return parent::hasMany('MaytmOrderDetail', 'order_id');
    }
}