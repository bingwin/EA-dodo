...
class AmazonUploadRelationQueuer extends  SwooleQueueJob{
...
//        $this->handelProductImages($accountId,$accountInfo);
//        exit;
//        $this->handelProduct($accountId,$accountInfo);
//        exit;

//          $this->handelProductPrice($accountId,$accountInfo);
//        exit;
        $this->testUpload(array('account_id' => $accountId,'feedSubmissionId' => '62691017539','type' => '_POST_PRODUCT_IMAGE_DATA_','id' => 16));
        exit;
//        $this->handelProductQuantity($accountId,$accountInfo);
//        exit;
                ...
                if($imageList){
                    ...
                    foreach($imageList as $sku => $imageInfo){
                            if(isset($imageInfo['main']) && $imageInfo['main']){
                                ...
                                $xml .= "<ImageLocation>{$imageInfo['main']['image_url']}</ImageLocation> ";
                            ...
                            if(isset($imageInfo['Swatch']) && count($imageInfo['Swatch'])){
                                foreach($imageInfo['Swatch'] as $swatchItem){
                                    ...
                                    $xml .= "<ImageLocation>{$swatchItem['image_url']}</ImageLocation> ";
                                    $xml .= "</ProductImage></Message>";
                                }
                            }
                        ...
                        if(isset($imageInfo['detail']) && count($imageInfo['detail'])){
                            foreach($imageInfo['detail'] as $key => $detailItem){
                                ...
                                $xml .= "<ImageLocation>{$detailItem['image_url']}</ImageLocation> ";
                                $xml .= "</ProductImage></Message>";
                            }
                        }
                    }
