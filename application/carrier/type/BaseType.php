<?php
namespace app\carrier\type;

use think\Exception;
use app\carrier\type\RepeatableType;
...
class BaseType
{
     ...
     *                'required' => false         The data is required if the required is true
     *            ]
     *
     */
    ...
    /**
     * @var boolean Whether check property type.
     */
    ...
    public function toArray()
    {
        ...
        foreach (self::$properties[get_class($this)] as $name => $info) {
            if (!array_key_exists($name, $this->values) && !empty($info['required'])) {
                $this->throwRequired(get_class($this));
    ...
    /**
     * Helper method that returns an associative json of the object's properties and values.
     *
     * @return string Associative string where the keys are the object's properties.
     */
    ...
    private function setValue($class, $name, $value)
    {
        ...
        if (!$info['repeatable']) {
            ...
            if ('array' !== $actualType) {
                $msg = "Invalid property type provided for ". $name;
    ...
    private static function ensurePropertyExists($class, $name)
    {
        if (!array_key_exists($name, self::$properties[$class])) {
            $msg = "The property of ". $name." does not exist";
            throw new Exception($msg);
        }
    }
    ...
    private static function ensurePropertyType($class, $name, $value)
    {
        ...
        foreach ($valid as $check) {
            if (self::checkPropertyType($check)) {
        ...
        if (!$isValid) {
            ...
            throw new Exception($name . '期望类型为' . $expectedType);
        }
    }
     ...
     */
    private function throwRequired($class)
    {
        $missing = [];
        foreach (self::$properties[$class] as $name => $info) {
            if (!isset($info['required']) || empty($info['required']) || array_key_exists($name, $this->values)) {
                continue;
            }
            $missing[] = $name;
        }
     ...
     *
     * @param string $name The name of the class the property belongs to.
     * @param string $value The of the attribute property.
     *
     * @return string The XML.
     */
    ...
    {
        if (is_subclass_of($value, '\app\carrier\type\BaseType', false)) {
    ...
    /**
     * Helper function when assigning values via the actor.
