...
class RepeatableType implements \ArrayAccess, \Countable, \Iterator
{
     ...
     * @throws Exception If the value is the wrong type for the property.
     */
    private function ensurePropertyType($value)
    {
        ...
        foreach ($valid as $check) {
            if (\app\carrier\type\BaseType::checkPropertyType($check)) {
                if ($check === $actualType) {
                    return;
                }
                $isValid = false;
        ...
        if (!$isValid) {
            throw new Exception("Invalid property type provided for ". $this->property);
        }
    }
}
