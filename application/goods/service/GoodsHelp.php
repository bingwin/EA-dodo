...
//            $result['platform_sale'] = $this->resolvePlatformSale($result['platform_sale']);
    ...
    public function getTPropertiesTextBySkuId($skuId)
    {
        ...
        return $this->getProTransPropertiesTxt($goodsInfo['transport_property']);
    }
    ...
    public function getSkuInfo($goods_id)
    {
        ...
        do {
            ...
            $sku_lists = GoodsSku::where(['goods_id' => $goods_id])->field('id,sku,thumb,goods_id,name,cost_price,retail_price,sku_attributes,weight,width,height,length,auto_update_time')->order('sku asc')->select();
            if (empty($sku_lists)) {
                break;
            }
...
//                    $value = $this->getAttributeValue($goods_attributes, $attribute_id, $attribute_value_id, $attribute_name);
//                    $row = [];
//                    $row['attribute_value'] = $value;
//                    $row['attribute_id'] = (int)$attribute_id;
//                    $sku['attributes'][] = $row;
...
//                        $this->pushSkuStatusQueue($sku_id, $status);
    ...
    public function pullCountDevelop($date = null)
    {
        ...
        foreach ($reset as $v) {
            ...
            $ret = $MonthlyTargetAmountService->addDevelopment($v['developer_id'], $v['quantity'], $year, $month);
            if ($ret !== true) {
    ...
    public function countDevelop($date = null)
    {
        ...
        $Model = new Goods();
        $reset = $Model->alias('g')
            ->field("count(*) as num,g.developer_id,g.channel_id,u.department_id")
    ...
    public function getGoodsTortDescription($id, $page, $page_size)
    {
        ...
        $ret = $ModelGoodsTortDescription->where('goods_id', $id)
            ->field('id,channel_id,account_id,site_code,remark,tort_time')
            ->page($page, $page_size)
            ->select();
    ...
    {
        $ModelGoodsTortDescription = new GoodsTortDescription();
        return $ModelGoodsTortDescription
            ->field('id,goods_id,channel_id,account_id,site_code,remark,tort_time')
            ->where('id', $id)
            ->find();
    }
    ...
    public function saveGoodsTortDescription($goods_id, $param, $user_id)
    {
        ...
        try {
            if (isset($param['id']) && $param['id']) {
                ...
                GoodsNotice::sendTortDescription($goods_id,$goodsInfo['spu'],$param['channel_id'],$param['site_code'],$accountName,$param['remark']);
            }
            return ['message' => '保存成功'];
        } catch (Exception $ex) {
            throw $ex;
    ...
    public function getGoodsTortDescriptionByGoodsId($aGoodsId)
    {
        $model = new GoodsTortDescription();
        $result = [];
        $ret = $model->where('goods_id', 'in', $aGoodsId)->field('id,goods_id,channel_id,account_id,site_code,remark,tort_time')->select();
        foreach ($ret as $v) {
            $result[$v['goods_id']][] = $v;
        }
        return $result;
     ...
     * 4、商品状态：停售状态的不参与分配
     */
    public function getAssignGoods($day)
    {
        //1.找出不分配的分类；
        $categoryModel = new Category();
        $categoryMainIds = [1, 7, 193, 197];
        ...
        $goods = Goods::where('platform&2=2')->where($where)->column('spu', 'id');
        //2.过滤任一平台存在侵权记录的；
        $goodsIds = array_keys($goods);
        if($goodsIds){
            $GoodsTortDescription = new GoodsTortDescription();
            $tortGoodsIds = $GoodsTortDescription->where(['goods_id' => ['in', $goodsIds]])->group('goods_id')->column('goods_id');
            foreach ($goods as $goods_id=>$spu) {
                if (in_array($goods_id, $tortGoodsIds)) {
    ...
    public function tortWhere($params)
    {
        ...
        if (isset($params['tortType']) && isset($params['tortText']) && !empty($params['tortText'])) {
            switch ($params['tortType']) {
                case 'select':
                    $where['remark'] = ['eq', $params['tortText']];
                    break;
                case 'text':
                    $where['remark'] = ['like', '%'.$params['tortText'].'%'];
                    break;
                default:
                    break;
            }
        ...
        if (isset($params['create_time_start']) || isset($params['create_time_end'])) {
            ...
            if (!empty($params['create_time_start']) && !empty($params['create_time_end'])) {
                ...
                $where['goods_tort_description.create_time'] = ['between', [strtotime($params['create_time_start']),strtotime($params['create_time_end'])+86399]];

            } elseif (!empty($params['create_time_start']) && empty($params['create_time_end'])) {

                $where['goods_tort_description.create_time'] = ['egt', strtotime($params['create_time_start'])];

            } elseif (empty($params['create_time_start']) && !empty($params['create_time_end'])) {

                $where['goods_tort_description.create_time'] = ['elt', strtotime($params['create_time_end'])];
