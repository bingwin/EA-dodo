...
use app\report\queue\ReportShortageExportQueue;
...
class ReportShortageService extends  AbsServer
{
    use Export;
    ...
    public function add($params)
    {
        try {
            ...
            return json(['message' => '新增成功', 'id' => $model->id], 200);
        } catch (\Exception $e) {
    ...
    public function applyExport($params)
    {
        ...
        $downLoadDir = '/download/express_shortage/';
        ...
        $listModel = new ReportShortageByDate();
        $count = $this->showDay($params)+1;
        if($count>1000){
            //加入队列
              Db::startTrans();
...
//                $da=new ReportShortageExportQueue();
//              $da->execute($params);
                (new CommonQueuer(ReportShortageExportQueue::class))->push($params);
                Db::commit();
    ...
    public function export(array $params)
    {
        ...
        try {
            ...
            $downLoadDir = '/download/express_shipped/';
            $saveDir = ROOT_PATH . 'public' . $downLoadDir;
    ...
    private function shippedExport( $params,$page,$pageSize)
    {
        ...
        if ($b_time && $e_time) {
            ...
            $where['dateline'] = ['EGT', strtotime($b_time)];
        } elseif ($e_time) {
            $where['dateline'] = ['ELT', strtotime($e_time)];
        }
        $sortField = 'dateline';
        $sortType = 'DESC';
        $field = 'warehouse_id,dateline,sum(quantity) as qty';
