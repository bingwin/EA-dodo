...
use app\common\model\vova\VomaAccount;
...
class VomaAccountService
{
    ...
    public function __construct()
    {
        if (is_null($this->vovaAccountModel)) {
            $this->vovaAccountModel = new VomaAccount();
        }
    }
    ...
    public function accountList(Request $request)
    {
        ...
        $accountList = $this->vovaAccountModel->field($field)->where($where)->order($orderBy)->page($page, $pageSize)->select();
        $new_array = [];
    ...
    public function save($data)
    {
        ...
        Db::startTrans();
        try {
            $this->vovaAccountModel->allowField(true)->isUpdate(false)->save($data);
            //获取最新的数据返回
            $new_id = $this->vovaAccountModel->id;
            //删除缓存
            Cache::store('vovaAccount')->delAccount();
            Db::commit();
        } catch (\Exception $e) {
            Db::rollback();
            throw new JsonErrorException($e->getMessage(), 500);
        }
        $accountInfo = $this->vovaAccountModel->field(true)->where(['id' => $new_id])->find();
        return $accountInfo;
    }
    ...
    {
        $accountInfo = $this->vovaAccountModel->field('id,code,name,vova_name,app_key,site,download_order,sync_delivery,download_listing')->where(['id' => $id])->find();
    ...
    public function update($id, $data)
    {
        ...
        if ($this->vovaAccountModel->isHas($id, $data['code'], '')) {
            throw new JsonErrorException('代码或者用户名已存在', 400);
        }
        $model = $this->vovaAccountModel->get($id);
        Db::startTrans();
        try {
            //赋值
            $model->code = isset($data['code'])?$data['code']:'';
            $model->name = isset($data['name'])?$data['name']:'';
            $model->vova_name = isset($data['vova_name'])?$data['vova_name']:'';
...
//            $this->lazadaAccountModel->allowField(true)->save($data, ['id' => $id]);
    ...
    public function update_download($ids, $data)
    {
        ...
        try {
            ...
            $this->vovaAccountModel->allowField(true)->update($param,['id' => ['in', json_decode($ids)]]);
            $model = $this->vovaAccountModel->field('id ,status,download_order,download_listing,sync_delivery')->where(['id' => ['in', json_decode($ids)]])->select();
            unset($ids);
            //删除缓存
            Cache::store('vovaAccount')->delAccount();
            Db::commit();
    ...
    {
        if (!$this->vovaAccountModel->check(['id' => $id])) {
            throw new JsonErrorException('账号不存在', 400);
        }
        try {
            $data['update_time'] = time();
            $this->vovaAccountModel->allowField(true)->save($data, ['id' => $id]);
            //删除缓存
            Cache::store('vovaAccount')->delAccount();
            return true;
        } catch (\Exception $e) {
    ...
    {
        $result = VovaApi::handler('common')->getToken($data);
        if ($result['state']) {
    ...
    public function refresh_token($id) {
        ...
        $vovaAccountModel = new VomaAccount();
        //检测账号的token
        $result = VovaApi::instance(['access_token' => $account['access_token']])->loader('common')->checkToken($account, true);
        ...
        if ($result && !empty($result['data'])) {
            ...
            //入库
            $vovaAccountModel->where(['id' => $account['id']])->update($temp);
            $cache->delAccount($id);
    ...
    public function authorization($id)
    {
        ...
        $result = $this->vovaAccountModel->field('app_key,app_secret')->where(['id' => $id])->select();
