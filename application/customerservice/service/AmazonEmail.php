...
*/
        ...
        $isSent = $this->sendMail($account, $info['buyer_email'], 0, $info['subject'], $content, $attachFile);
        if ($isSent) {
            //统计数据；
...
//            Db::rollback();
        ...
        $isSent = $this->sendMail($account, $receivedMail['sender'], 0, $subject, $content, $attachFile);
        //发送成功
        if ($isSent) {
...
//                    $groupOld = Cache::store('AmazonEmail')->getGroupData($groupWhere);
                    ...
                    //如果是发件箱邮件或者系统转发邮件，发件人和收件人反过来
                    if ($sent_box_mail || $system_forwarding_mail)
                    {
...
//        $emailAccount = EmailAccounts::where('id', $mail['email_account_id'])->find();
        ...
        $isSent = $this->sendMail($account, $mail['receiver'], $mailId, $mail->subject, $content->content, $mail->attachments);
        if ($isSent) {
