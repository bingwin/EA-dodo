...
class JumiaAccountService
{
    ...
    public function add($data, $uid = 0)
    {
        try {
            ...
            $this->jumiaAccountModel->add($save_data);
    ...
    {
        $field = 'id,account_name,code,is_invalid status,create_time,update_time,is_authorization,sync_delivery,download_order,download_listing,jumia_enabled,client_secret,client_id';
        if ($id == 0) {
