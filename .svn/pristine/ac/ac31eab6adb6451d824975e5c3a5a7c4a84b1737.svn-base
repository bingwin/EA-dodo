...
use app\common\model\walmart\WalmartOrder as WalmartOrderModel;


class WalmartOrder extends Cache
{
    private $key = 'table:walmart:order:';
    private $set = 'table:walmart:order:set';
    private $orderSyncTime = 'order_sync_time';

    /** 获取属性信息
     * @param accountId 账号id
     * @param string $order_number walmart订单id
     * @param array $data
     * @return array|mixed
     */
...
//            $this->redis->hset($key, $order_number, json_encode($data));
    ...
    {
        $key = 'hash:WalmartOrderUpdateTime';
        $list = WalmartOrderModel::field('id,order_number,last_update_time')->where(['last_update_time' => ['gt', time() - 3*24*3600]])->select();
    ...
    {
        $this->redis->hSet('hash:WalmartOrderRefundLogs', $key, json_encode($data));
    }
    ...
    {
        if ($this->redis->hExists('hash:WalmartOrderRefundLogs', $key)) {
            return true;
        }
        return false;
    }
    ...
    {
        $this->redis->zAdd('hash:WalmartOrderLogs', $key, json_encode($data));
    }
    ...
    {
        $result = [];
        if($this->persistRedis->exists('hash:WalmartOrderLogs')) {
            $result = $this->persistRedis->zRange('hash:WalmartOrderLogs', $start, $end);
        }
        return $result;
    }
    ...
    {
        $this->redis->zAdd('hash:WalmartOrderToLocalLogs', $key, json_encode($data));
    }
    ...
    {
        $result = [];
        if($this->persistRedis->exists('hash:WalmartOrderToLocalLogs')) {
            $result = $this->persistRedis->zRange('hash:WalmartOrderToLocalLogs', $start, $end);
        }
        return $result;
    }
    ...
    {
        $this->redis->zAdd('hash:WalmartSynchronousLogs', $key, json_encode($data));
    }
    ...
    {
        $result = [];
        if($this->persistRedis->exists('hash:WalmartSynchronousLogs')) {
            $result = $this->persistRedis->zRange('hash:WalmartSynchronousLogs', $start, $end);
        }
        return $result;
    }
