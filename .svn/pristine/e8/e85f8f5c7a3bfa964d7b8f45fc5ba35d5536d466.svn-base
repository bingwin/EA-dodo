...
use app\common\model\pdd\PddOrder as CdOrderModel;


class PddOrder extends Cache
{
    private $key = 'table:pdd:order:';
    private $set = 'table:pdd:order:set';
    private $orderSyncTime = 'order_sync_time';

    /** 获取属性信息
     * @param accountId 账号id
     * @param string $order_number cd订单id
     * @param array $data
     * @return array|mixed
     */
    ...
    {
        $key = 'hash:CdOrderUpdateTime';
        $list = CdOrderModel::field('id,order_number,last_update_time')->where(['last_update_time' => ['gt', time() - 3*24*3600]])->select();
    ...
    {
        $this->redis->hSet('hash:CdOrderRefundLogs', $key, json_encode($data));
    }
    ...
    {
        if ($this->redis->hExists('hash:CdOrderRefundLogs', $key)) {
            return true;
        }
        return false;
    }
    ...
    {
        $this->redis->zAdd('hash:CdOrderLogs', $key, json_encode($data));
    }
    ...
    {
        $result = [];
        if($this->persistRedis->exists('hash:CdOrderLogs')) {
            $result = $this->persistRedis->zRange('hash:CdOrderLogs', $start, $end);
        }
        return $result;
    }
    ...
    {
        $this->redis->zAdd('hash:CdOrderToLocalLogs', $key, json_encode($data));
    }
    ...
    {
        $result = [];
        if($this->persistRedis->exists('hash:CdOrderToLocalLogs')) {
            $result = $this->persistRedis->zRange('hash:CdOrderToLocalLogs', $start, $end);
        }
        return $result;
    }
    ...
    {
        $this->redis->zAdd('hash:CdSynchronousLogs', $key, json_encode($data));
    }
    ...
    {
        $result = [];
        if($this->persistRedis->exists('hash:CdSynchronousLogs')) {
            $result = $this->persistRedis->zRange('hash:CdSynchronousLogs', $start, $end);
        }
        return $result;
    }
