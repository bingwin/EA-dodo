...
//            }
    ...
    public static function addItem($id, $userId=0)
    {
        $fieldList = 'id,listing_type,listing_sku,account_id,site,currency,variation,paypal_emailaddress,primary_categoryid,
            ...
            mod_style,mod_sale,return_time,assoc_order,sku,local_sku,is_virtual_send';
...
//            EbayListing::update(['listing_status'=>EbayPublish::PUBLISH_STATUS['publishing']],['id'=>$list['id']]);//维护状态
            ...
            foreach ($imgs as $img) {
                ...
                } elseif (strpos($img['eps_path'], '/$_3.JPG') === false) {//图片像素过低
                    $failImg = 1;
     ...
     * @throws \think\exception\DbException
     */
    public static function formatItem($data)
    {
        $item = [];
        $list = $data['list'];
        $set = $data['set'];
        ...
        $item['ApplicationData'] = $set['application_data'];
        //立即付款
        ...
        if ($list['disable_buyer']) {//开启了买家限制
            $brd = json_decode($set['buyer_requirment_details'],true);
        ...
        if ($hasVariant) {//多属性需要过滤掉变体的属性
            $varKey  = array_keys(json_decode($variants[0]['variation'],true));
        ...
        //付款方式
        $paymentMethods = json_decode($set['payment_method'],true);
        if (!$paymentMethods) {
...
//        $item['PrivateListing'] = $list['private_listing'];
        ...
        $productListingDetails['BrandMPN']['Brand'] = $set['brand'];
        $productListingDetails['BrandMPN']['MPN'] = $set['mpn'];
        if (!$hasVariant) {//单属性
            $productListingDetails['EAN'] = $set['ean'];
            $productListingDetails['ISBN'] = $set['isbn'];
            $productListingDetails['UPC'] = $set['upc'];
        }
        ...
        if ($hasVariant) {
            ...
            $vsSet = [];
            foreach ($variants as $variant) {
                $variation['Quantity'] = $variant['v_qty'];
                ...
                //变体属性
                $vs = json_decode($variant['variation'],true);
                $varSpec = [];
                $varNameValueListI = 0;
        ...
        if ((float)$list['vat_percent'] > 0) {
            $item['VATDetails']['VATPercent'] = $list['vat_percent'];
        }
        return $item;
    }
    ...
    public static function UploadSiteHostedPictures(&$imgs, $config, $multi=false)
    {
        ...
        $api = $packApi->createApi($config,'UploadSiteHostedPictures',$config['site_id']);
