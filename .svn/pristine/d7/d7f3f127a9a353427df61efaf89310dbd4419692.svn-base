...
//            'account_id' => $ebayAccount['id'],
    ...
    public function send($params)
    {
        ...
        $isSent = $this->sendMail($account, $info['buyer_email'], $recordId, $info['subject'], $content, $attachment);
        if ($isSent) {
            return true;
        } else {
    ...
    public function sendMail(EmailAccount $account, $customerEmail, $sentId, $subject, $content, $attachFile)
    {
        try{
            ...
            $isSent = $sender->send($customerEmail, $subject, $content, $attachFile);
...
//        $accountInfo = EmailAccounts::where(['id' => $email_accountId])->find();
        ...
        if(strtoupper($params['method']) == 'RE'){
            $isSent = $this->sendMail($account, $receivedMail['sender'], $recordId, $subject, $content, $attachFile);
        }else{
            foreach ($receiver as $rec) {
                $isSent = $this->sendMail($account, $rec, $recordId, $subject, $content, $attachFile);
                if (!$isSent) {
                    throw new Exception($this->mailSendError, 400);
                }
            }
...
//            Cache::handler()->hSet('hash:email_sync_log:' . MailReceiver::$syncingEmailAccount, date('YmdHis'), $ex->getMessage());
    ...
    private function check_infringement_email($subject, $account_code, $account_id, $sync_time)
    {
        ...
        //侵权邮件而且是最近三天
        if (preg_match($patten, $subject, $match) && $sync_time > $report_time) {
...
//        $emailAccount = EmailAccounts::where('id', $mail['email_account_id'])->find();
        ...
        $account = new EmailAccount(
            ...
            $emailService['smtp_port'],
            $platform);
        $isSent = $this->sendMail($account, $mail['receiver'], $mailId, $mail->subject, $content->content, $mail->attachment);
        if ($isSent) {
            return true;
        } else {
            throw new Exception($this->mailSendError, 400);
...
//        }
    ...
    public function ReceiverMailsAddr($params){
        ...
        $emailList = $this->ebayEmailModel->where($where)->field('receiver')->group('receiver')->select();
        return $emailList;
    }
    ...
    public function SenderMailsAddr($params){
        ...
        $emailList = $this->ebayEmailModel->where($where)->field('sender')->group('sender')->select();
        return $emailList;
    }
