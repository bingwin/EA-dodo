...
use app\report\queue\ReportShippedExportQueue;
...
class ReportShippedService extends  AbsServer
{
    use Export;
   ...
   public  function showDay($params){
    ...
    public function add($params)
    {
        try {
            ...
            if (empty($params['warehouse_id'])) {
                return json(['message' => '仓库不能为空'], 400);
            }
            if (empty($params['channel_id'])) {
                return json(['message' => '平台 不能为空'], 400);
            }
            if (empty($params['quantity'])) {
                return json(['message' => '数量 不能为空'], 400);
            }
    ...
    public function applyExport($params)
    {
        ...
        try {
            ...
            $downLoadDir = '/download/report_shipped1/';
            ...
            $listModel = new ReportShippedByDate();
            $count =$this->showDay($params)+1;
            if ($count > 1000) {
                //加入队列
                Db::startTrans();
                try {
                    ...
                    (new CommonQueuer(ReportShippedExportQueue::class))->push($params);
                    Db::commit();
    ...
    protected function createExportFileName($params)
    {
        if (!empty($params['time_start']) && !empty($params['time_end'])) {
            ...
            $fileName = '已发货记录报表（' . $date_b . '-' . $data_e . ')';
        } else {
            $fileName = '已发货记录报表';
        }
        return $fileName;
