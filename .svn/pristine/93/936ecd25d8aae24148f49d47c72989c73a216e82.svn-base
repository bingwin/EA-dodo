            ...
            foreach($publishData['attr'] as $k=>$value)
            {
                if(isset($value['attrValueId'])&&is_array($value['attrValueId']))
                {
                    ...
                    if(isset($value['attrName']) && $value['attrName']){
                        $value['attrName'] = trim($value['attrName']);
                    }

                    if(isset($value['attrValue']) && $value['attrValue']){
                        $value['attrValue'] = trim($value['attrValue']);
                    }
...
//                        $update=true;
    ...
    {
        //$AliexpressCategory = AliexpressCategory::where('category_pid',$intCategoryId)->field(['create_time','update_time'],true);
        $field = '`category_id`,`category_pid`,`category_level`,`category_name_zh`,`category_isleaf`,`update_status`,`required_size_model`';
        $AliexpressCategory = AliexpressCategory::where('category_pid','=',$intCategoryId)->field($field);
        /**获取有权限分类
        if($intCategoryId === 0 && $intAccountId!==0)
        {
            $objPower =  AliexpressAccountCategoryPower::field('category_id')->where('account_id',$intAccountId)->select();
           
            $strPower = join(',',array_column($this->ObjToArray($objPower),'category_id'));
           
            $AliexpressCategory->where('category_id','IN',$strPower);
        
        }
         **/
        return  $AliexpressCategory->select();
    }
