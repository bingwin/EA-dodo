...
use app\index\service\FummartAccountService;
use think\Db;
use think\Exception;
use think\Request;
 ...
 * @author fummart
 */
class FummartAccount extends Base
{
    protected $fummartAccountService;

    public function __construct()
    {
        parent::__construct();
        if(is_null($this->fummartAccountService)){
            $this->fummartAccountService = new FummartAccountService();
        }
    }
    ...
    public function index(Request $request)
    {
        try{
            ...
            $response = $this->FummartAccountService->accountList($params,$page,$pageSize);
    ...
    public function add(Request $request)
    {
        ...
        try{
            ...
            $uid = $user['user_id'];
            $response = $this->fummartAccountService->add($params,$uid);

            if($response === false) {
                return json(['message' => $this->fummartAccountService->getError()], 400);
            }
     ...
     * @method GET
     * @param  int $id
     * @url /fummart-account/:id
     * @return \think\response\Json
     */
    public function read($id)
    {
        $response = $this->fummartAccountService->getOne($id);
        return json($response);
    }
     ...
     * @method GET
     * @param  int $id
     * @url /fummart-account/token/:id
     * @return \think\response\Json
     */
    public function getToken($id)
    {
        $response = $this->fummartAccountService->getTokenOne($id);
        return json($response);
    }
    ...
    public function updateToken(Request $request)
    {
        try{
            ...
            $response = $this->fummartAccountService->refresh_token($params, $uid);
            if($response === false) {
                return json(['message' => $this->fummartAccountService->getError()], 400);
            }
     ...
     * @title 停用，启用账号
     * @method post
     * @url /fummart-account/states
     */
    public function changeStatus(Request $request)
    {
        $params = $request->param();
        $response = $this->fummartAccountService->changeStatus($params);
        if($response === false) {
            return json(['message' => $this->fummartAccountService->getError()], 400);
        }
