...
//                }
            ...
            if(isset($response['success']) && isset($response['attributes']))
            {
                ...
                if($attributes)
                {
                    foreach($attributes as $attribute)
                    {
                        ...
                        {
                            $attributeModel->save($data,['category_id'=>$category_id,'id'=>$attribute['id']]);
                        }else{
                            $attributeModel->save($data);
                        }
            ...
            if(isset($response['success']) && isset($response['attributes']) && $response['attributes'])
            {
                ...
                if($attributes)
                {
                    foreach($attributes as $attribute)
                    {
                        ...
                        if($attribute['id']==2)
                        {
                            if(isset($attribute['values']['aeopAttrValueDto']) && $attribute['values']['aeopAttrValueDto'])
                            {
    ...
    public function  managerAttributeValue($value=[],$attr_id=0,$category_id=0)
    {
        ...
        if(!is_array($value))
        {
            ...
            foreach ($value as $name => $v)
            {
                ...
                {
                    $model->allowField(true)->isUpdate(true)->save($attr_val,['id'=>$v['id']]);
                }else{
                    $model->allowField(true)->isUpdate(false)->save($attr_val);
                }
            ...
            if(isset($response['success']) && isset($response['aeopPostCategoryList']))
            {
                ...
                if($aeopPostCategoryList)
                {
                    ...
                    foreach($aeopPostCategoryList as $category)
                    {
                        ...
                        if($model->where('category_id','=',$category['id'])->find())
                        {
                            $model->allowField(true)->save($cate,['category_id'=>$category['id']]);
                        }else{
                            $model->allowField(true)->save($cate);
                        }
    ...
    public  function postProduct($job)
    {
        ...
        try{
            ...
            //产品分组ID
            $post['groupId']=$product['group_id'] ? $product['group_id'] : 0;
...
//            ];
     ...
     * @return mixed
     */
    public function managerDetail(&$detail,$account,$code)
    {
        try{
            if($detail)
            {
    ...
    public  function combineRelationCustomDescription($product,$detail,$account,$code)
    {
        try{
            ...
            if(count($match)>1 && !empty($match[1]))
            {
                ...
                $new_iamges = $this->uploadImageURLs($account,$images,$code,1);
                if(isset($new_iamges['result']) && $new_iamges['result']===true){
                    $bankImages = explode(';',$new_iamges['data']);
                    foreach ($match[1] as $i=>$img)
                    {
     ...
     * @return string
     */
    public  function uploadOneImage($account,$image,$code)
    {
        ...
        if(strpos($image,'alicdn.com')==false)
        {
            ...
            $image = $this->translateImgToFullPath($image, $code);
            $param['url']=$image;
            $params = array_merge($account,$param);
            $response = AliexpressService::execute($params);
     ...
     * @return string
     */
    public  function uploadImageURLs($account,$images,$code,$local=0)
    {
        try{
            $param['module']='product';
            $param['class']='Photobank';
                ...
                foreach($images as $image)
                {
                    ...
                    if(strpos($image,'alicdn.com')==false)
                    {
                        ...
                        if(isset($response['photobankUrl']) && $response['photobankUrl'])
                        {
                            ...
                            $imageError = strpos($image,'http://') ===false && strpos($image,'https://') ===false ? 'http://14.118.130.19/'.$image : $image;
                            break;
                        }
                    }else{
                        $returnUrl[]=$image;
    ...
    public  function uploadImage($account,$images,$code,$local=0)
    {
        $param['module']='product';
        $param['class']='Photobank';
        $param['action']='uploadimageforsdk';
        ...
        foreach($images as $image)
        {
            ...
            if(strpos($image,'alicdn.com')==false)
            {
                if($local==1)
                {
                    $image = $this->translateImgToLoalPath($image, $code);
                }elseif($local==0){
                    $image = $this->translateImgToFullPath($image, $code);
                }
